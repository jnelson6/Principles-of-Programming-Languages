...
type exp_val =
  | NumVal of int
  | BoolVal of bool
  | PairVal of exp_val*exp_val    (* 1/3 THIS IS WHAT WE ADD FOR QUIZ 3 *************)
...
let pair_of_PairVal : exp_val -> (exp_val*exp_val) ea_result =  function       (* 2/3 THIS IS WHAT WE ADD FOR QUIZ 3 *************)
  |  PairVal(ev1,ev2) -> return (ev1,ev2)
  | _ -> error "Expected a pair!"
           
let string_of_expval = function                               (* 3/3 THIS IS WHAT WE ADD FOR QUIZ 3 *************)
